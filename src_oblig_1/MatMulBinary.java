...
public class MatMulBinary {
  ...
  public static int[][] importMatrix(DataInputStream sc, int height, int width)
             throws Exception {
    int A[][] = new int[height][width];
    for (int i=0; i<height; ++i) {
      for (int j=0; j<width; ++j) {
          if (sc.available() > 0) {
            A[i][j] = (int)(sc.readByte() * Math.pow(256, 0) + sc.readByte() * Math.pow(256, 1) +
            sc.readByte() * Math.pow(256, 2) + sc.readByte() * Math.pow(256, 3));
          } else {
