...
readBinaryData:
   mov eax,SYS_READ        ; SYS_READ = 3 -> EAX system read call
   mov ebx,STDIN           ; STDIN -> EBX innput from keyboard/file
   mov ecx, w32FrStck(1)   ; matrix adress -> ECX so we know where to put our first data
   mov edx, 8*4            ; define size: 8 digit with size 4 bytes = 32 bits -> EDX 
   int 80h                 ; int = interupts current program runs 80h program
   mov eax, 8*4            ; 32 -> EAX store 32 size of 8 digits that is already in matrix in eax for later use
   mov ecx, w32FrStck(1)   ; matrix adress -> ECX
   add ecx, eax            ; (add) EAX + ECX -> ECX move 32 further so we can read new data, and not erase what we added priviously
   mov w32FrStck(1), ecx   ; ECX -> w32FrStck(1) we update new position in the stack
   mov edx, w32FrStck(2)   ; size of matrix example 4*l*n -> EDX
   sub edx, eax            ; (subtract) size of matrix - 32 "stored value" size of 8 digits -> EDX 
   mov w32FrStck(2), edx   ; updating size in stack to new size since 32 bits are already occupied
   cmp edx, 0              ; compare new size to 0, jump to readBinaryData "run method once again" if cmp return greater than 0
   jg readBinaryData       ; jumps to begining of method if cmp returned greater than 0
   ret                     ; ends when cmp returns 0 or lower...
...
jTLoop:
   ...
   mov ecx, w32FrStck(1)  ; acc
   mul ecx                ; eax <- acc*m[y][x]
   mov ebx, w32FrStck(5)  ; ebx <- w
   mov edx, 0             ; edx <- 0 (reset)
...
l3:
   ...
   inc eax                 ; k++
   mov w32FrStck(6), eax   ; overwrite updated k
...
l2:
   ...
   setupMatrixIndexing w32FrStck(7), w32FrStck(0), w32FrStck(5), w32FrStck(4)
                           ; m     ,     w       ,     i       ,     j
   
   mov edx, w32FrStck(3)   ; acc -> edx
   ...
   inc eax                 ; j++
   mov w32FrStck(4), eax   ; overwrite/update j
...
l1:
   ...
   mov eax, 0              ;
   mov w32FrStck(4), eax   ; overwrite updated j
   ;update new value i

   mov eax, w32FrStck(5)   ; i -> eax
   inc eax                 ; i++
   mov w32FrStck(5), eax   ; overwrite updated k
